...
void v_add(double* x, double* y, double* z){
   ...
   {   int NumThread= omp_get_num_threads();
       ...
       int Stride=NumThread;

       for(int i=CurrThread;i<ARRAY_SIZE; i=i+Stride){
           z[i] = x[i] + y[i];
       }
   }
}
...
//       int NumThread= omp_get_num_threads();
//       int CurrThread=omp_get_thread_num();
//       int Stride=ARRAY_SIZE/NumThread;
//
//
//       for(int i=CurrThread*Stride;i<(CurrThread+1)*Stride; i++){
//             z[i] = x[i] + y[i];
//       }
//
//       if (ARRAY_SIZE%NumThread!=0){
//         for (int i=(Stride/NumThread*NumThread);i<ARRAY_SIZE;i++){
//           z[i]=x[i] + y[i];
//         }
//       }
//     }
