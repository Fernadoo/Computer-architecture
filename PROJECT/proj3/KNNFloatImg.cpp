...
//   x += M_PI/2;
//   x -= double((int(x>M_PI) & int((2*M_PI)*1000000000000))/1000000000000);
//   return mysin(x);
// }
...
float KNN_removalValue(const float * imgIn, const unsigned int xIn, const unsigned int yIn, float * kernel, const unsigned int width,  const unsigned int height, const unsigned int kernelSize, const unsigned int k, const float distance_thr){
  ...
  //sort(kernel,kernel + kernelSize * kernelSize);
  nth_element(kernel,kernel+k,kernelSize*kernelSize+kernel-1);

  // get the k nearest vaild neighbor
  int start = 0;
  // for(int i = 0; i < kernelSize * kernelSize && i < start + k; i++){
  //   if(kernel[i] == 0.f){
  //     start = i + 1;
  //   }else{
  //     k_sum += kernel[i];
  ...
  // get the mean of the k neighbors
  mean = k_sum / k;
