...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
  ...
  /* use strtok() to read next token */
    ...
    /* if the whole line is comments */
    if (!token){
      continue;
    }
  /* deal with label */
    int is_label = add_if_label(line, token, (byte_offset+4) , symtbl);
    if (is_label == -1){
      err_count += 1;
      /**/
  ...
  /* parse the instrution */
    ...
    int write_line = write_pass_one(output, name, args, num_args);
    if (write_line > 0){
      byte_offset += 4*(write_line);
    }else{
      /*printf("name:%s%d\n",name ,line);*/
      err_count += 1;
    }
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    /* Use translate_inst() to translate the instruction and
       write to output file. */
        int err = translate_inst(output, name, args, num_args, (byte_offset+4), symtbl, reltbl);

    /* If an error occurs,
       the instruction will not be written and you should call
         raise_instruction_error().  */
        if (err == -1){
          raise_instruction_error(line, name, args, num_args);
          err_count += 1;
          printf("288 err:%d\n",err_count );
        }else{
          byte_offset += 4;
