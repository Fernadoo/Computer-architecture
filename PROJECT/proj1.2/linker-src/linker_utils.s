...
inst_needs_relocation:
	# YOUR CODE HERE
	srl	$a0, $a0, 26 # take out the opcode
	li	$t2, 2 # j's opcode is 02
	beq	$a0, $t2, needs_relocation
	li	$t2, 3 # jal's opcode is 03
	beq	$a0, $t2, needs_relocation
	li	$v0, 0 # not need, return 0
	jr	$ra
needs_relocation:
	li	$v0, 1 # need, return 1
	jr 	$ra
...
relocate_inst:
	# YOUR CODE HERE
	addiu	$sp, $sp, -20 # create 8 bytes spaces
	sw	$ra, 0($sp) # store the return naddr
	sw	$a0, 4($sp) # store the inst
	sw	$a1, 8($sp) # store the offset
	sw	$a2, 12($sp) # store the symtbl
	sw	$a3, 16($sp) # store the reltbl
	#----check addr in reltbl
	move	$a0, $a1 # $a0 for addr to look for
	move	$a1, $a3 # $a1 for reltbl
	jal	symbol_for_addr # call func to find symbol in reltbl
	li	$t2, 0
	beq	$v0, $t2, not_found_it # return null, not found
	#----check name in symtbl
	lw	$a0, 12($sp) # $a0 for symtbl
	move	$a1, $v0 # $a1 for name to look for
	jal	addr_for_symbol # call to find addr in symtbl
	li	$t2, -1
	beq	$v0, $t2, not_found_it # return -1, not found
	#----main body
	sll	$t2, $v0, 4 # take off the PC
	srl	$t2, $t2, 6 # take off 00
	lw	$t3, 4($sp) # load the inst
	srl	$t3, $t3, 26
	sll	$t3, $t3, 26 # clean the lower 26 bits
	#----create the new inst
	or	$v0, $t3, $t2 # 6 + 26
	#----return
