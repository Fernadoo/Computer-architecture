...
write_machine_code:
	...
	#-------------mod by me
	sw $a2, 24($sp) # sp+24 is symtbl
	sw $a3, 28($sp) # sp+28 is reltbl
	#-------------mod by me	
...
write_machine_code_next_inst:
	...
	# YOUR_INSTRUCTIONS_HERE
	move	$a0, $v1 # move the inst to be parsed into $a0
	li	$a1, 16 # set the base to 16
	jal	parse_int 
	move	$s7, $v0 # store the hex number into $s7
	
	# 4. Check if the instruction needs relocation. If it does not, branch to
	# the label write_machine_code_to_file:
	# YOUR_INSTRUCTIONS_HERE
	move	$a0, $s7 # move the hex number into $a0
	jal	inst_needs_relocation # judge it need rel
	beq	$v0, $zero, write_machine_code_to_file # if not, write out
	...
	# YOUR_INSTRUCTIONS_HERE
	move	$a0, $s7 # inst to rel
	move	$a1, $s6 # byte offset
	lw	$a2, 24($sp) # stmtbl
	lw	$a3, 28($sp) # reltbl
	jal	relocate_inst
	move	$s7, $v0
...
write_machine_code_to_file:
	...
	# YOUR_INSTRUCTIONS_HERE 
	move	$a0, $s7 # hex int 
	la	$a1, hex_buffer # string to write
	jal	hex_to_str 
