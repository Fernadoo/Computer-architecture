...
hex_to_str:
	# YOUR CODE HERE
	li	$t2, 28 # 7*4 = 28 the shift amount
loop_hex_to_str:
	srlv	$t3, $a0, $t2 # shift 28, 24, 20... 0
	andi	$t4, $t3, 0xF # take out the lower 5 bits
	bgt	$t4, 9, hex_a_f # if >9, deal with 10-15
	addiu	$t4, $t4, 48 # 0 + 48 = 48 = '0'
 	j	write_hex_string
 hex_a_f:
 	addiu	$t4, $t4, 87 # 10 + 87 = 97 = 'a'
 write_hex_string:
 	sb	$t4, 0($a1) # write into $a1
 	addiu	$a1, $a1, 1 # increace the buffer ptr by 1
 	addiu	$t2, $t2, -4 # decreace the shift amount by 1
 	bge	$t2, $zero, loop_hex_to_str # still >= 0, back to loop
 	#----end the string with '\n' '\0'
 	li	$t2, 10 # plus '\n'
 	sb	$t2, 0($a1)
 	li	$t2, 0 # plus '\0'
